...
def simulate(filehandle, cif_path, idx= None, gpu_id=0, clean_up=False):
    ...
    sim_params = get_sim_params(sp, grid_steps=np.array([4,4]), orientation_num=20)
    ...
    for y_dir, (index, z_dir) in zip(y_dirs, enumerate(z_dirs)):
        try:
            t = time()
            # build supercell
            sp.build_unit_cell()
            ...
            wrong_shape = cbed.shape != (16, 512, 512) or proj_potential.shape != (1, 512, 512)
            if has_nan or wrong_shape:
                print('rank=%d, skipped simulation=%s, index=%d, error=NaN' % (comm_rank, cif_path, index))
            else:
                # write to h5 / tfrecords / lmdb
                g = filehandle.create_group('sample_%d_%d' % (idx, index))
                ...
                g.attrs['time'] = time() - t
                g.attrs['semi_angle'] = sim_params['semi_angles'][index]
                g.attrs['d_hkl'] = sim_params['d_hkl'][index]
                filehandle.flush() 
                print('rank=%d, finished simulation=%s, index=%d' % (comm_rank, cif_path, index))

    ...
    with env.begin(write=delete, buffers=True) as txn:
        for idx in records:
            ...
            try:
                ...
                if delete:
                    image = np.random.uniform(low=0.0, high=1e-4, size=data_specs['image_shape']).astype(data_specs['image_dtype'])
                    label = np.random.uniform(low=0.0, high=100., size=data_specs['label_shape']).astype(data_specs['label_dtype']) 
